...
class PostgresDataframeIOManager(IOManager):
    def __init__(self, uid: str, pwd: str, server: str, db: str, port: str) -> None:
        ...
        self.server = server
        self.port = port
        self.engine = create_engine(f'postgresql://{self.uid}:{self.pwd}@{self.server}:{self.port}/{self.db}')
        try: 
            with self.engine.connect() as conn:
                if not conn.dialect.has_schema(conn, "dbt_project"):
                    conn.execute(sqlalchemy.schema.CreateSchema("dbt_project"))
                    conn.commit()
        except:
            pass 
    ...
    def handle_output(self, context: OutputContext, obj: pd.DataFrame):
        ...
        obj.to_sql(table_name, self.engine, schema='dbt_project',if_exists='replace', index=False)
